...
public class AlquileresImpl implements AlquileresService {
    ...
    public Alquileres iniciarAlquiler(String idCliente, int estacionRetiroId, int estacionDevolucionId) {
        ...
        nuevoAlquiler.setEstacionDevolucion(estacionDevolucionId);
...
/*
    // Método para obtener el porcentaje de descuento según el día promocional
    private double obtenerDescuentoPorDiaPromocional(LocalDateTime fechaHoraRetiro) {
        // Lógica para obtener el porcentaje de descuento según el día promocional
        // Puedes consultar tu base de datos para obtener esta información
        // En este ejemplo, asumimos que hay un 10% de descuento para todos los días promocionales
        return 0.10;
    }

    private double calcularDistancia(int estacionRetiro, int estacionDevolucion) {
        // Implementa la lógica para calcular la distancia entre estaciones.
        // Puedes usar fórmulas matemáticas o cualquier otra lógica adecuada.
        return 0.0; // Reemplaza con la implementación real
    }


    private long calcularDuracionEnMinutos(LocalDateTime inicio, LocalDateTime fin) {
    ...
    private List<Tarifas> obtenerTarifas() {
        return tarifasRepository.findAll();
    }

    private double calcularCostoTotal(Alquileres alquiler, double distanciaEnKm) {
        // Obtener las tarifas correspondientes a la fecha y hora de retiro
        List<Tarifas> tarifas = obtenerTarifas();

        // Calcular la duración del alquiler en minutos
        long duracionEnMinutos = calcularDuracionEnMinutos(alquiler.getFechaHoraRetiro(), alquiler.getFechaHoraDevolucion());

        // Calcular el costo fraccionado por minuto hasta el minuto 30
        double costoMinutosFraccionados = duracionEnMinutos <= 30 ? duracionEnMinutos * tarifas.get(0).getMontoMinutoFraccion() : 30 * tarifas.get(0).getMontoMinutoFraccion();

        // Calcular el costo por hora completa a partir del minuto 31
        double costoHorasCompletas = Math.ceil((duracionEnMinutos - 30) / 60.0) * tarifas.get(0).getMontoHora();

        // Calcular el costo adicional por kilómetro
        double costoKm = distanciaEnKm * tarifas.get(0).getMontoKm();

        // Calcular el costo total sumando todas las partes
        double costoTotal = tarifas.get(0).getMontoFijoAlquiler() + costoMinutosFraccionados + costoHorasCompletas + costoKm;
    ...
    @Override
    public Alquileres finalizarAlquiler(int id, String moneda) {
        // Obtener la información del alquiler
        Alquileres alquiler = getById(id);
